<template>
  <MegaMenu :categories="categoryTree">
    ...
    <template v-if="viewport.isGreaterOrEquals('lg')">
      <nav class="hidden ml-4 md:flex md:flex-row md:flex-nowrap">
        ...
        >
          <template #prefix>
            <NuxtImg :src="cartIcon" width="17" height="15" alt="shopping cart" /><span class="text-sm">{{ cartItemsCount }}</span>
          </template>
        </UiButton>
      </nav>
    </template>

    <template v-if="viewport.isGreaterOrEquals('lg') && localeCodes.length > 1">
        <UiButton
    ...
    <div v-if="viewport.isLessThan('lg')">
      <UiButton
        class="group relative hover:!bg-header-400 active:bg-header-400 rounded-md !gap-1 pl-1"
        :tag="NuxtLink"
        :to="localePath(paths.wishlist)"
        :style="{ color: iconColor }"
        :aria-label="t('numberInWishlist', { count: wishlistItemIds.length })"
      ...
      <UiButton
        class="group relative hover:!bg-header-400 active:!bg-header-400 -ml-0.5 rounded-md !gap-1"
        :tag="NuxtLink"
        :style="{ color: iconColor }"
        :to="localePath(paths.cart)"
        :aria-label="t('numberInCart', { count: cartItemsCount })"
      ...
      >
        <template #prefix>
          <NuxtImg :src="cartIcon" width="17" height="15" alt="shopping cart icon" /><span class="text-sm">{{ cartItemsCount }}</span>
        </template>
      </UiButton>
    </div>
  </MegaMenu>
  ...
  <NuxtLazyHydrate when-idle>
    <SfModal
      ...
      class="w-full h-full md:h-fit md:top-4 md:w-[700px] z-50"
      tag="section"
      role="dialog"
